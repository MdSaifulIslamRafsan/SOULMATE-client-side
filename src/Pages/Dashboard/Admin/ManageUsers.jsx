...
const ManageUsers = () => {
    ...
    const {data: usersData , refetch} = useQuery({
        queryKey: ['users'],
        queryFn: () => axiosSecure.get('/boiDatas')
        .then((res)=>{
            return res.data;
        })
    });
    ...
    const columns =[
        ...
        {
            name: 'Make admin',
            selector: user => user.role === 'admin' ? 'admin' : <AwesomeButton onPress={() =>handleMakeAdmin(user)} type="primary">Make admin</AwesomeButton>,
        },
        {
            name: 'Make premium',
            selector: user => user.role === 'premium' ? 'premium' : <AwesomeButton onPress={()=>handleMakePremium(user)} type="primary">Make premium</AwesomeButton>,
        }]
